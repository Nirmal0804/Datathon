...
const ANOMALY_THRESHOLDS = {
  ANOMALY_GROWTH_THRESHOLD: 15.0,
  CATEGORY_SHIFT_THRESHOLD: 10.0,
  ANOMALY_SCORE_THRESHOLD: 60,
  FREQUENCY_DEVIATION_THRESHOLD: 20.0,
};
...
export default function AnalystHotspotLayout({ onNavigate }) {
  ...
  useEffect(() => {
    const fetchHotspots = async () => {
      ...
      try {
        ...
        const items = (res?.hotspots || []).map((h, idx) => ({
          hotspotRank: idx + 1,
          ...
          trend: 'Persistent',
          densityIndex: Math.min(10, (h.fir_count / 5)).toFixed(1),
          growthPercentage: 0,
          lastIncidentDate: new Date().toISOString().slice(0, 10),
          historicalAverage: Math.round(h.fir_count * 0.7),
          recommendation: 'Monitoring and patrol recommended based on FIR density.',
          activitySummary: `${h.fir_count} FIRs recorded. Dominant crime: ${h.dominant_crime_type || 'Unknown'}. Districts: ${(h.districts || []).join(', ')}.`,
        }));
        setAllHotspots(items);
        if (items.length > 0 && !selectedHotspot) {
          setSelectedHotspot(items[0]);
          setComparisonTargetId(items.find(h => h.hotspotId !== items[0]?.hotspotId)?.hotspotId || '');
        }
      } catch (err) {
  ...
  const filteredHotspots = useMemo(() => {
    ...
    if (filters.district !== 'All') list = list.filter(h => h.district === filters.district);
    if (filters.riskLevel !== 'All') list = list.filter(h => h.riskLevel === filters.riskLevel);
    if (filters.crimeCategory !== 'All') list = list.filter(h => h.dominantCrime === filters.crimeCategory);
    if (filters.trend !== 'All') list = list.filter(h => h.trend === filters.trend);
    return list;
  }, [allHotspots, filters, searchQuery]);
  ...
  useEffect(() => {
    if (selectedHotspot) {
      const nextTarget = allHotspots.find(h => h.hotspotId !== selectedHotspot.hotspotId);
      if (nextTarget) {
  ...
  const comparisonTarget = useMemo(() => {
    return allHotspots.find(h => h.hotspotId === comparisonTargetId);
  }, [comparisonTargetId]);
