  ...
  return (
    <div className="bg-white border border-[#E7ECF3] rounded-[22px] p-6 shadow-sm h-full flex flex-col justify-between">
      <div className="flex items-center gap-3 border-b border-[#F1F5F9] pb-4 mb-5 shrink-0">
        ...
        <div>
          <h3 className="text-sm font-black text-[#0B1F4D] uppercase tracking-wider mb-1">{t('dashboard.emergingHotspots', 'Emerging Hotspots')}</h3>
          <p className="text-xs font-semibold text-[#64748B]">{t('hotspots.hotspotsNeedingAttention', 'High-activity zones needing attention')}</p>
        </div>
      </div>
          ...
          >
            ...
            <div className="mb-4">
              ...
              <div className={`flex items-center gap-1 text-xs font-bold ${hs.trend.startsWith('+') ? 'text-rose-600' : 'text-emerald-600'}`}>
                ...
                {hs.trend} <span className="text-[#64748B] font-semibold text-[10px] uppercase tracking-wider ml-1">{t('common.thisWeek', 'This Wk')}</span>
              </div>
            </div>
            ...
            <button 
              ...
              className="flex items-center justify-center gap-1.5 w-full py-2 bg-[#F8F9FB] hover:bg-[#F1F5F9] rounded-xl text-xs font-bold text-[#0B1F4D] transition-colors border border-[#E7ECF3] cursor-pointer"
            >
              <Map className="w-3.5 h-3.5" />
              {t('hotspots.viewOnMap', 'View Map')}
            </button>
          </motion.div>
        ))}
      </div>
