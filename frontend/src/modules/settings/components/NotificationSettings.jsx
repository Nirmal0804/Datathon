...
const Toggle = ({ value, onChange }) => (
  <button
    ...
    className={`relative inline-flex w-10 h-5 rounded-full transition-colors shrink-0 cursor-pointer ${value ? 'bg-primary' : 'bg-slate-700'}`}
  >
...
export default function NotificationSettings() {
  ...
  const categories = [
    { key: 'anomaly', label: t('dashboard.recentAlerts', 'Anomaly Alerts'), desc: 'Statistical spikes or ML-detected outliers.' },
    { key: 'hotspot', label: t('nav.hotspots', 'Hotspot Updates'), desc: 'Emerging or shifting crime hotspot zones.' },
    { key: 'report', label: t('reports.readyToExport', 'Report Ready'), desc: 'Notify when a generated report is available.' },
    { key: 'system', label: t('nav.systemHealth', 'System Notices'), desc: 'Platform updates, model retraining events.' },
  ];
