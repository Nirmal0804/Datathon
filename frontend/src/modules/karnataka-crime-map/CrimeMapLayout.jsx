...
export default function CrimeMapLayout({ role = 'analyst' }) {
  ...
  const [analyticsOpen, setAnalyticsOpen] = useState(isAnalyst);
  ...
  const [layers, setLayers] = useState({
    showMarkers: true,
    showHeatmap: isAnalyst,
    showClusters: isAnalyst,
    showDensity: false,
  ...
  const loadCases = useCallback(async (f) => {
    ...
    try {
      ...
      if (f.district && f.district !== 'All') params.district = f.district;
      if (f.policeStation && f.policeStation !== 'All') params.station_id = f.policeStation;
      if (f.category && f.category !== 'All') params.crime_head = f.category;
      if (f.status && f.status !== 'All') params.status = f.status;
      if (f.startDate) params.start_date = f.startDate;
      if (f.endDate) params.end_date = f.endDate;

      const res = await getFieldMapCases(params);
      ...
      console.error('Failed to load crime map data:', err);
      setAllCases([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const filteredCases = useMemo(() => {
    ...
    return list;
  }, [allCases, filters.severity, timelineCutoff, isAnalyst]);
  ...
  const handleResetAll = () => {
    ...
    loadCases({ district: 'All', policeStation: 'All', category: 'All', severity: 'All', status: 'All', startDate: '', endDate: '' });
  };
  ...
  const caseMetrics = useMemo(() => {
    ...
    const districtCases = allCases.filter(c => c.district === selectedCase.district);
  ...
  return (
    <div className="flex h-full w-full relative overflow-hidden bg-slate-950 rounded-xl border border-slate-800 shadow-xl">
      ...
      {/* 5. District Risk Intelligence Drawer - BLOCKED_ML: predictive risk data not available */}
      <AnimatePresence>
        {false && (
          <></>
        )}
