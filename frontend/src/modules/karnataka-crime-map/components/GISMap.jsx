...
export default function GISMap({
  ...
  useEffect(() => {
    fetch('/data/hotspot_summaries.json')
      .then((res) => {
        if (!res.ok) throw new Error('Failed to fetch GIS hotspot summaries');
        return res.json();
      })
      .then((data) => {
        if (Array.isArray(data) && data.length > 0) {
          const formatted = data.map((hs) => {
            const level = hs.Total_Crimes > 30 ? 'Critical' : 'High';
            const color = level === 'Critical' ? '#ef4444' : '#f59e0b';
            return {
              district: `${hs.Primary_District} (Cluster #${hs.Cluster_ID})`,
              center: [hs.Centroid_Latitude, hs.Centroid_Longitude],
              count: hs.Total_Crimes,
              dominantCat: hs.Primary_Crime_Head,
              level,
              color
            };
          });
          setMlHotspots(formatted);
        }
      })
      .catch(() => {
        // Fallback to computed dynamic hotspots
      });
  }, []);

  // Compute dynamic hotspots from the mock dataset or use ML DBSCAN centroids
  const dynamicHotspots = useMemo(() => {
    if (mlHotspots && mlHotspots.length > 0) return mlHotspots;
